...
import AddModal from '@/pages/chargeBaby/batchBaby/modals/addModal';
import EditModal from '@/pages/chargeBaby/batchBaby/modals/editModal';
...
@Form.create()
class BatchBaby extends Component {
  state = {
    payload: {
      condition: {
        batchName: '',
  ...
  initialData = type => {
    ...
    dispatch({
      type: 'chargeBaby/getBatchBabyList',
      payload: {
        condition: {
          batchName: '',
        },
  ...
  handleAddOk = values => {
    const { dispatch } = this.props;
    dispatch({
      type: 'chargeBaby/operateBatchBaby',
      payload: values,
      callback: res => {
  ...
  handleDelete = () => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'chargeBaby/deleteBatchBaby',
  ...
  render() {
    ...
    const { list } = this.props.chargeBaby.batchBabyList;
    ...
    const columns = [
      ...
      { title: '批次名称', dataIndex: 'batchName', width: 300 },
      { title: '订单总量（套）', dataIndex: 'orderNum', width: 150 },
      {
        title: '订单金额（元）',
      ...
      { title: '已到货', dataIndex: 'allNum', render: text => (text ? text : 0), width: 100 },
      { title: '未出库设备', dataIndex: 'waitExport', width: 120 },
      { title: '已出库设备', dataIndex: 'alreadyExport', width: 120 },
      {
        title: '下单日期',
        dataIndex: 'orderDate',
        width: 200,
