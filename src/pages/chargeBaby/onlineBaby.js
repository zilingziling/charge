...
@Form.create()
class OnlineBaby extends Component {
  state = {
    details: {},
    activeKey: '1',
    current: 1,
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      const start = fieldsValue['start'];
      const end = fieldsValue['end'];
      this.setState(
        {
          id: id,
          phone: phone,
            ...
            <Row>
              <Col className={styles.col}>
                <Form className={styles.form}>
                  <Form.Item className={styles.form} label="小宝编号">
                    {getFieldDecorator('id', {})(
                      <Input placeholder="请输入小宝编号进行搜索" className={styles.input} />
                    )}
                  </Form.Item>
                  ...
                  <Form.Item className={styles.form} label="租借次数">
                    {getFieldDecorator('start', {})(<InputNumber min={0} />)}
                  </Form.Item>
                  <span className={styles.space}> - </span>
                  <Form.Item className={styles.form}>
                    {getFieldDecorator('end', {})(<InputNumber min={0} />)}
                  </Form.Item>
                </Form>
                <Button
                  type="primary"
