...
import PersonalModal from '@/pages/User/personalAgent/modals/personalModal';
...
class Index extends Component {
  ...
  handleSearch = () => {
    ...
    const { payload } = this.state;
    validateFields(['level', 'state', 'phone'], (errors, values) => {
      if (!errors) {
        payload.condition = { ...values, userType: 0 };
        payload.nowPage = 1;
        dispatch({
          type: 'userManage/personalAgent',
          payload,
  ...
  handleClickEdit = info => {
    const { userType, realname, level, mobile, area, reward, id } = info;
    this.setState({
      editInfo: {
        userType,
        realname,
  ...
  render() {
    ...
    const { list } = this.props.userManage.personalAgent;
    const { pagination, addVisible, selectedRowKeys } = this.state;
    const pager = {
      ...pagination,
      showTotal: () => {
        return (
    ...
    const columns = [
      ...
      { title: '分润比例', dataIndex: 'reward', width: 100, render: text => `${text}%` },
      {
        title: '累计订单总额（元）',
