...
import { recover } from '@/services/record';
...
class Scrapped extends Component {
  ...
  handleRenew = deviceId => {
    recover(deviceId).then(res => {
      if (res.code === '000000') {
        message.success('操作成功');
        this.getRecord();
      } else message.error('操作失败', 6);
    });
  };
  render() {
    let z = this;
    ...
    const dataSource = this.props.record.scrapped ? this.props.record.scrapped.list : [];
    const columns = [
      {
        title: '操作',
        dataIndex: 'operation',
        render: (text, record) => (
          <span>
            <a href="javascript:;" onClick={() => this.handleRenew(record.deviceId)}>
              启用
            </a>
          </span>
        ),
