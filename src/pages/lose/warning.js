...
class Warning extends Component {
  ...
  render() {
    ...
    const dataSource = this.props.lose.warning ? this.props.lose.warning.list : [];
    ...
    const columns = [
      {
        ...
        render: (text, record) => (
          <span>
            <Popconfirm
              title={`是否遗失编号为${record.deviceId}的设备？`}
              onConfirm={() => this.returned(record)}
            >
              <a href="javascript:;">遗失设备</a>
            </Popconfirm>
          ...
          <TabPane tab="详情页" key="2" className={styles.tabPane}>
                ...
                total={() => (
                  <span
                    dangerouslySetInnerHTML={{
                      __html: this.toDecimal(this.state.details.rentPriceSum / 100) || 0,
                    }}
                  />
                )}
              />
            ...
            <Row justify="start" type="flex" className={styles.line}>
              ...
              <Col className={styles.right}>
                设备单价 : {this.toDecimal(this.state.details.details / 100) || '无'}
              </Col>
            </Row>
            <Row justify="start" type="flex" className={styles.line}>
              <Col className={styles.left}>
